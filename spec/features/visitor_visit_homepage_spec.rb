...
feature 'Visitor visit homepage' do
  ...
  scenario 'and view recipes list' do
    ...
    recipes.take(4).each do |recipe|
      ...
      expect(page).not_to have_css('li',
                                   text: "Cozinha: #{recipe.cuisine.name}")
      expect(page).not_to have_css('li',
                                   text: "Tipo: #{recipe.recipe_type.name}")
    end
  end
  ...
  scenario 'and see the top 3 favorites' do
    ...
    recipes.values_at(0, 1, 5).each do |recipe|
      expect(page).to have_css('div.most_favorited',
                               text: recipe.title)
      expect(page).to have_css('div.most_favorited',
                               text: "Cozinha: #{recipe.cuisine.name}")
      expect(page).to have_css('div.most_favorited',
    ...
    recipes.values_at(2, 3, 4, 6, 7, 8, 9).each do |recipe|
      ...
      expect(page).not_to have_css('div.most_favorited',
                                   text: "Cozinha: #{recipe.cuisine.name}")
      expect(page).not_to have_css('div.most_favorited',
                                   text: "Tipo: #{recipe.recipe_type.name}")
    end
